;;; punctuation.el --- Punctuation replacing -*- lexical-binding: t; -*-
...
;; URL: https://github.com/twlz0ne/punctuation
...
(defun punctuation--convert-punctuations (&optional looking-fn looking-next-p)
  "Convert full-width/half-width panctuations before or around point.
...
if LOOKING-NEXT-P is not nil, stop looking next."
  (when (funcall (or looking-fn 'looking-back) "\\s-*\\([[:punct:]]\\)\\s-*")
    (let ((replace (cdr (assoc (match-string-no-properties 1) punctuation-paires))))
      (when replace
        (save-excursion
          (if (listp replace)
                  ...
                  (pcase replace
                    (`(:replace ,_ :next-fn looking-back)
                     (save-excursion
                       ...
                       (punctuation--convert-punctuations 'looking-back t)))
                    (`(:replace ,_ :next-fn looking-at)
                     (save-excursion
                       (punctuation--convert-punctuations 'looking-at t))))))
            (replace-match replace nil nil nil 1)))))))

(defun punctuation ()
  "Command to call."
  (interactive)
  (punctuation--convert-punctuations 'looking-back))
